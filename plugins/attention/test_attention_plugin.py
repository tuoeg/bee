...
# just matmul
def attention_cpu(buffer_h):
    _x1 = buffer_h[0]
    _x2 = buffer_h[1]

    _1 = np.matmul(_x1, _x2)
    print(_1)
    return _1

def getAttentionPlugin():
...
def run():
    ...
    inputTensorList = []
    inputTensorList.append( network.add_input('x1', trt.float32, [-1,nSL,nEmbedding]) )
    inputTensorList.append( network.add_input('x2', trt.float32, [nEmbedding, nEmbedding]) )

    profile = builder.create_optimization_profile()
    profile.set_shape('x1',[1,nSL,nEmbedding],[4, nSL,nEmbedding],[6, nSL,nEmbedding])
    config.add_optimization_profile(profile)

    pluginLayer = network.add_plugin_v2(inputTensorList, getAttentionPlugin())
    ...
    print('num_bindings is %d, to checkout non-dynamic axis neet set binding\n', engine.num_bindings)
    ...
    # x2
    bufferH.append( np.random.rand(nEmbedding, nEmbedding).astype(np.float32).reshape(nEmbedding, nEmbedding) * 2 - 1)
    #output
